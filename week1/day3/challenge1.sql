...
select c.id,c.name,sum(o.amount) as total_amount from customers c inner join orders o on c.id=o.customer_id group by c.id;

--- task2
select c.id,c.name,sum(o.amount) as total_amount,r.region_name
from customers c 
inner join orders o 
...
select c.id,c.name from orders o inner join customers c on c.id=o.customer_id group by c.id having count(*)>2;
